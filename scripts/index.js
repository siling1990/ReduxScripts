                    ...
                    FileUtil.readFile(`${PROJECTPATH + pageFolderName}/${FILENAV}`, (error, data) => {
                        if (error) {
                            ...
                            if (data && !!data.toString()) {
                                ...
                                FileUtil.writeFile(`${PROJECTPATH + pageFolderName}/${FILENAV}`, steNav, 'utf8')
                            } else {
                                console.log(`${PROJECTPATH + pageFolderName}/${FILENAV}\t读取失败`)
                            }
                        }
...
function init() {
    //创建目录
    FOLDERARRY.push('Store')
    for (let i = 0; i < FOLDERARRY.length; i++) {
        FileUtil.mkdirs(PROJECTPATH + FOLDERARRY[i], (error) => {

        })
    }
    FileUtil.fileExist("../model/ReduxStore.js", (exists) => {
        if (exists) {
            FileUtil.readFile("../model/ReduxStore.js", (error, data) => {
                if (error) {
                    console.log(error)
